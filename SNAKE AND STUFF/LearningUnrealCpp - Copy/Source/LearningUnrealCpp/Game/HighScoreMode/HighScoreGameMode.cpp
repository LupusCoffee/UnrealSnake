...
void AHighScoreGameMode::BeginPlay()
{
	...
	if (HighScoreGameState)
	{
		...
		HighScoreGameState->OnGateCreated.AddDynamic(this, &ThisClass::AHighScoreGameMode::GateCreated);
	}
...
void AHighScoreGameMode::GateCreated(ARollaBallItemBase* Gate)
{
	if (HighScoreGameState)
	{
		HighScoreGameState->AddGate(Gate);
	}
}
